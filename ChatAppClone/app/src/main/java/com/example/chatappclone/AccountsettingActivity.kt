...
class AccountsettingActivity : AppCompatActivity()
{
    ...
    override fun onCreate(savedInstanceState: Bundle?)
    {
        ...
        change_image_text.setOnClickListener {
            checker = "clicked"
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?)
    {
        ...
        if (requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE  &&  resultCode == Activity.RESULT_OK  &&  data != null)
        {
            ...
            imageUri = result.uri
            picture_edit.setImageURI(imageUri)
        }
    }
    ...
    private fun updateUserInfoOnly()
    {
        when {
            ...
            User_name.text.toString() == "" -> Toast.makeText(this, "Please write user name first.", Toast.LENGTH_LONG).show()
            bio.text.toString() == "" -> Toast.makeText(this, "Please write your bio first.", Toast.LENGTH_LONG).show()
            else -> {
                val usersRef = FirebaseDatabase.getInstance().reference.child("Users")

                val userMap = HashMap<String, Any>()
                userMap["fullname"] = full_name_profile_frag.text.toString().toLowerCase()
                userMap["username"] = User_name.text.toString().toLowerCase()
        ...
        usersRef.addValueEventListener(object : ValueEventListener
        {
            override fun onDataChange(p0: DataSnapshot)
            {
                if (p0.exists())
                {
                    ...
                    Picasso.get().load(user!!.getImage()).placeholder(R.drawable.profile1).into(picture_edit)
